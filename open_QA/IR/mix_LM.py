...
class Block(nn.Module):
    ...
    def forward(self, x, attention_mask, at=None):
        ## Attention
        h = x
        if not self.ln_pos:
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        for layer_block in self.layer:
            hidden_states, at = layer_block(hidden_states, attention_mask, at)
...
class BertMultiTask_ICT(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        if self.train_mode == 'ict':
            ...
            #target = torch.arange(0, q_input_ids.size(0)).to(torch.long).to(q_input_ids.device)
